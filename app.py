...
with tab1:
    ...
    # 1. SECTION ENERGIE (Titres épurés sans le mot "Consommation")
    ...
    with col_en_1:
        fig_elec = px.bar(df, x="Collège", y="Électricité (kWh)", color="Collège", 
                          title="Électricité (kWh)", text_auto=True,
                          color_discrete_sequence=px.colors.qualitative.Pastel)
        st.plotly_chart(fig_elec, use_container_width=True)
        
    with col_en_2:
        fig_gaz = px.bar(df, x="Collège", y="Gaz (m3)", color="Collège", 
                         title="Gaz (m3)", text_auto=True,
                         color_discrete_sequence=px.colors.qualitative.Pastel)
        st.plotly_chart(fig_gaz, use_container_width=True)
        
    st.divider()
    
    # 2. SECTION CANTINE (Mise à jour en kg pour les serviettes)
...
with tab2:
    if st.session_state.auth:
        ...
        with st.form("form_saisie"):
            ...
            serviettes = c5.number_input("Serviettes en papier utilisées (kg)", min_value=0.0, step=0.1, value=current_data["Serviettes papier (kg)"] if current_data else 0.0)
            ...
            # Nom du bouton modifié selon ta demande
            ...
            if submit:
                ...
                # Message de validation demandé + effet de célébration !
